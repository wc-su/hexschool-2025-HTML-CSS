...
function toggleCarouselAutoplay() {
  if (window.innerWidth >= mdBreakpoint) {
    carousel.pause(); // 停止自動輪播
  } else {
    carousel.cycle(); // 啟動自動輪播
  }
}
...
function resetMarginLeft() {
  ...
  // console.log(`${topList.style.marginRight} / ${topMarginRight}`);
  ...
  // console.log(`${recommendList.style.marginRight} / ${recommendMarginRight}`);
  ...
  if (window.innerWidth >= mdBreakpoint) {
    ...
    if((topRect.left === 0 && topMarginRight !== 0) ||
      (topRect.left > 0)) {
    ...
    if((recommendRect.left === 0 && recommendMarginRight !== 0) ||
      (recommendRect.left > 0)) {
